...
class Visit(db.Model):
    ...
    datetime = db.Column(
        db.DateTime(timezone=True), nullable=False, default=convert_datetime_to_local
    )
    ...
    def query_by_interval(start: date, end: date):
        return Visit.query.filter(
            cast(Visit.datetime, Date) == start, cast(Visit.datetime, Date) == end
        )
    ...
    def total_by_date(start: str, end: str):
        ...
        return (
            ...
            .filter(Visit.datetime.between(start, end))
            .group_by("date")
            .order_by(asc("date"))
        )
    ...
    def total_by_year_month(year: int, month=None):
        ...
        if month is None:
            return (
                ...
                .filter(extract("year", Visit.datetime) == year)
                .group_by("date")
            )
    ...
    def visits_by_ip(ips: list = None):
        ...
        if list is None:
            query = (
                db.session.query(Network, func.count(Network.id).label("views"))
                .join(Visit.network)
                .group_by(Network)
            )
        else:
...
class Network(db.Model):
    ...
    question_created_ip = db.relationship(
        "Question",
        backref="question_created_network",
        lazy="dynamic",
        foreign_keys="[Question.question_network_id]",
    )
    answer_created_ip = db.relationship(
        "Question",
        backref="answer_created_network",
        lazy="dynamic",
        foreign_keys="[Question.answer_network_id]",
    )
    questions_views = db.relationship(
        "QuestionView",
        cascade="all, delete-orphan",
        single_parent=True,
        backref="network",
    ...
    created_user = db.relationship(
        "User",
        backref="created_network",
        lazy="dynamic",
        foreign_keys="[User.created_network_id]",
    )
    last_login_user = db.relationship(
        "User",
        backref="last_login_network",
        lazy="dynamic",
        foreign_keys="[User.last_login_network_id]",
    )
    current_login_user = db.relationship(
        "User",
        backref="current_login_network",
        lazy="dynamic",
        foreign_keys="[User.current_login_network_id]",
    )
    confirmed_user = db.relationship(
        "User",
        backref="confirmed_network",
        lazy="dynamic",
        foreign_keys="[User.confirmed_network_id]",
    )
    notifiers = db.relationship(
        "Notifier", backref="network", lazy="dynamic", single_parent=True
    )
    visits = db.relationship(
        "Visit", backref="network", lazy="dynamic", single_parent=True
    )
    history_answer_created_ip = db.relationship(
        "QuestionHistory",
        backref="answer_created_network",
        lazy="dynamic",
        foreign_keys="[QuestionHistory.answer_network_id]",
    )
    history_network = db.relationship(
        "QuestionHistory",
        backref="network",
        lazy="dynamic",
        foreign_keys="[QuestionHistory.network_id]",
    )
...
class FilePDF(db.Model):
    ...
    topics = db.relationship(
        "Topic",
        secondary=file_topic,
        backref=db.backref(
            "files", lazy="dynamic", cascade="save-update", single_parent=True
        ),
        lazy="dynamic",
    )
...
class FileView(db.Model):
    ...
    def views_by_file_pdf(self, file_pdf_id: int):
        return (
            ...
            .filter(self.file_pdf_id == file_pdf_id)
            .scalar()
        )
    ...
    @staticmethod
    def query_by_month_year(year: int, month: int):
        return FileView.query.filter(
            extract("year", FileView.datetime) == year,
            extract("month", FileView.datetime) == month,
        )
    ...
    def query_by_interval(start: date, end: date):
        return FileView.query.filter(
            cast(FileView.datetime, Date) == start, cast(FileView.datetime, Date) == end
        )
