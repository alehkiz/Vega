...
def index():
    ...
    if current_user.is_authenticated and current_user.is_support:
        query = (
            ...
            .join(Question.view)
            .filter(Question.answer_approved == True, Question.active == True)
    ...
    if not sub_topic is False:
        paginate = (
            ...
            .filter(Question.sub_topics.contains(sub_topic))
            .paginate(per_page=app.config.get("QUESTIONS_PER_PAGE"), page=page)
        )
    else:
        paginate = (
            query.group_by(Question)
...
def search():
    ...
    if g.question_search_form.validate():
        ...
        )  # .join(QuestionView.question, full=True).filter(Question.answer_approved==True).order_by(QuestionView.count_view.desc())

        paginate = q.paginate(
            per_page=app.config.get("QUESTIONS_PER_PAGE", 1), page=page
        )
        ...
        iter_pages = list(paginate.iter_pages())
        first_page = (
            iter_pages[0] if len(iter_pages) >= 1 else None
        )  # url_for('.search',page=iter_pages[0], q= g.question_search_form.q.data)
        last_page = (
            paginate.pages if paginate.pages > 0 else None
        )  # url_for('.search',page=iter_pages[-1] if iter_pages[-1] != first_page else None, q= g.question_search_form.q.data)
        if paginate.total == 0:
            form = CreateQuestion()
            topic = Topic.query.filter(
                Topic.name.ilike(session.get("AccessType"))
            ).first()
            if topic is None:
                return abort(505)
            if form.validate_on_submit():
                question = Question.query.filter(
                    Question.question.ilike(form.question.data)
                ).first()
                if not question is None:
                ...
                if not form.errors:
                    if current_user.is_authenticated:
                        ...
                        user = User.query.filter(
                            User.id == app.config.get("USER_ANON_ID", False)
                        ).first()
                    ...
                    try:
                        ...
                        db.session.rollback()
                        app.logger.error(
                            app.config.get("_ERRORS").get("DB_COMMIT_ERROR")
                        )
                        app.logger.error(e)
                        return abort(500)
...
def view(id=None):
    ...
    if current_user.is_anonymous:
        ...
        if not question.has_topic(g.topic):
            ...
            flash(
                Markup(
                    f"A pergunta que você tentou acessar está disponível para <b>{question.topic_name}</b>, selecione o módulo correto para acessar a pergunta."
                ),
                category="danger",
            )
...
def edit(id):
    ...
    if form.validate_on_submit():
        qh = QuestionHistory()
        qh.add_history_user_id = current_user.id
...
def activate(id):
    ...
    if confirm != "true":
        return jsonify({"status": "error", "message": "not confirmed"}), 404
...
def active(id):
    ...
    try:
        ...
        q.update_user_id = current_user.id
        db.session.commit()
        return jsonify({"id": q.id, "status": "success"})
    except Exception as e:
        app.logger.error(app.config.get("_ERRORS").get("DB_COMMIT_ERROR"))
        app.logger.error(e)
        db.session.rollback()
        ...
        if not form.errors:
            ...
            if current_user.is_admin:
                if form.approved.data is True:
                    question.answer_approved = form.approved.data
                    question.answer_approved_at = convert_datetime_to_local(
                        datetime.utcnow()
                    )
            ...
            try:
                ...
                db.session.rollback()
                return render_template(
                    "add.html", form=form, title="Incluir dúvida", question=True
                )
        else:
...
def answer(id: int):
    ...
    if form.validate_on_submit():
        if not q is None:
            if q.id != id:
                form.question.errors.append(
                    "Você alterou a pergunta para uma já cadastrada"
                )
...
def approve(id: int):
    ...
    if not q.was_answered:
        flash(
            "Questão ainda não respondida, responda primeiro para aprovar",
            category="danger",
        )
    ...
    if form.validate_on_submit():
        if (
            not Question.query.filter(
                Question.question.ilike(form.question.data.lower())
            ).first()
            is None
        ):
            if q.id != id:
...
def tag(name):
    ...
    paginate = (
        db.session.query(Question)
        .filter(Question.tags.contains(tag), Question.answer_approved == True)
        .join(Question.topics)
...
def topic(name, type):
    ...
    if type in ["pendente", "aprovada"]:
        if type == "pendente":
            paginate = topic.questions.filter(
...
def like_action(question_id):
    ...
    if action is False or action not in ["like", "unlike"]:
        return jsonify({"status": "error", "message": "ação inválida"}), 404
    if action == "like":
        if question.is_liked(current_user.id):
            return (
                jsonify(
                    {"status": "error", "message": "Usuário já gostou dessa questão"}
                ),
                404,
            )
        ...
        return jsonify({"status": "success", "message": "Ação concluída"}), 200
    if action == "unlike":
        if not question.is_liked(current_user.id):
            return (
                jsonify(
                    {"status": "error", "message": "Usuário não curtiu essa questão"}
                ),
                404,
            )
        ...
        if rs is False:
            return (
                jsonify(
                    {"status": "error", "message": "Não foi possível remover o gostar"}
                ),
                404,
            )
        return jsonify({"status": "success", "message": "Ação concluída"}), 200
...
def save_action(question_id):
    ...
    if action == "save":
        if question.is_saved(current_user.id):
            return (
                jsonify(
                    {"status": "error", "message": "Usuário já salvou dessa questão"}
                ),
                404,
            )
    ...
    if action == "unsave":
        if not question.is_saved(current_user.id):
            return (
                jsonify(
                    {"status": "error", "message": "Usuário não salvou essa questão"}
                ),
                404,
            )
...
def make_question():
    ...
    if form.validate_on_submit():
        ...
        if question.count() > 0:
            flash(
                "Já temos perguntas cadastras sobre o assunto pesquisado!!",
                category="warning",
            )
        ...
        if not form.errors:
            ...
            try:
                db.session.commit()
                flash(
                    f"Dúvida cadastrada com sucesso! ID: {question.id}",
                    category="success",
                )
                return redirect(url_for("question.index"))
            except Exception as e:
