...
class NotifierForm(FlaskForm):
    title = StringField('Título', validators=[DataRequired('Item obrigatório'), Length(min=5, max=200, message='O titulo deve conster entre 5 e 200 caracteres')])
    content = TextAreaField('Conteúdo', validators=[DataRequired('Item obrigatório'), Length(min=5, max=1000, message='O titulo deve conster entre 5 e 1000 caracteres')])
